...
class WebSocketServer:
    def __init__(self, host='0.0.0.0', port=8080):
        ...
        print(f"\n{'=' * 50}")
        print(f"   P2P WEBSOCKET SERVER v2.0.0")
        print(f"{'=' * 50}")
        print(f"\n🚀 Launched at ws://{host}:{port}")
    ...
    def handle_client(self, conn, addr, peer_id):
        ...
        print(f"✅ [CONNECTION] Peer {peer_id} от {addr[0]}:{addr[1]}")
        print(f"   👥 Online: {list(clients.keys())}")
        ...
        try:
                ...
                try:
                    ...
                    if target:
                        ...
                        if target_int in clients:
                            ...
                            self.send_message(conn, json.dumps({
                                'type': 'error',
                                'message': f'Peer {target} is offline. Available: {list(clients.keys())}'
                            }))
                            print(f"❌ Peer {target} is not found. Available: {list(clients.keys())}")
            ...
            if peer_id in clients:
                del clients[peer_id]
            conn.close()
            print(f"\n❌ [DISCONNECTION] Peer {peer_id}")
            print(f"   👥 Reamins: {list(clients.keys()) if clients else 'nobody'}\n")
